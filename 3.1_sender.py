...
def sender(message):
    # Step 1: Convert each character in the message to its ASCII value
    values = [ord(char) for char in message]  # Convert chars to ASCII values
    
    # Step 2: Compute the sum of all ASCII values
    total_sum = sum(values)
    
    # Step 3: Convert the sum to binary and extract the parts for XOR
    bin_sum = format(total_sum, 'b')  # Convert sum to binary string
    
    # Step 4: XOR the first 4 bits with the bits past the 4th bit
    if len(bin_sum) > 4:
    ...
    # Step 5: Calculate the checksum as the binary complement of the wrapped sum
    checksum = (~wrapped_sum) & 0xF  # Binary complement within 4 bits
    
    # Append checksum to the packet
    packet = values + [checksum]
    
    print(f"Sender -> Message: {message}, ASCII Values: {values}, Decimal Sum: {total_sum}, Wrapped Sum: {wrapped_sum}, Checksum: {checksum}")
    return packet
...
if __name__ == "__main__":
    message_to_send = "PKA_NSCOM03"  # The word we are using for the checksum
    packet = sender(message_to_send)  # Generate packet with checksum
